...
class DbClientRepository extends AbstractDbRepository implements ClientRepositoryInterface
{
                ...
                if (\is_string($field)) {
                    if($conditionValue!="NULL"){
                        if($condition == "BETWEEN"){
                            $from = $i."from";
                            $to = $i."to";
                            $where[] = "(app_clients.$field $condition :value$from AND :value$to)";
                            $params[":value$from"] = $value[1];
                            $params[":value$to"] = $value[2];
                        }else{
                            $where[] = "(app_clients.$field $condition :value$i)";
                            $params[":value$i"] = $condition==" LIKE "?"%$value%":$value;
                        }
                    }else{
                        $where[] = "(app_clients.$field $condition)";
                    }
                    $i++;
                    ...
                    foreach ($field as $f) {
                        if($conditionValue!="NULL"){
                            if($condition == "BETWEEN"){
                                $from = $i."from";
                                $to = $i."to";
                                $w[] = "(app_clients.$f $condition :value$from AND :value$to)";
                                $params[":value$from"] = $value[1];
                                $params[":value$to"] = $value[2];
                            }else{
                                $w[] = "(app_clients.$f $condition :value$i)";
                                $params[":value$i"] = $condition==" LIKE "?"%$value%":$value;
                            }
                        }else{
                            $w[] = "(app_clients.$f $condition)";
                        }
                        $i++;
                    }
                    $where[] = '(' . implode(' OR ', $w) . ')';
    ...
    public function insert(Client $client): int
    {
        $cmd = $this->db->createCommand('INSERT INTO app_clients
                     ...
                     VALUES (:id, :email, :emailVerified, :name, :surname, :phone, :phoneVerified, :workPhone, :otherPhone, :countryId, :state, :city, :street, :building, :flat, :createdBy, :responsible, :createdAt, NOW(), :zip, :skype)',
